...
type IteratorInterface interface {
	// Moves iterator to next position in a list. Returns true if next position exists else false.
	// This is also the validity state of the iterator. If list is not empty and Next() returns false,
	// to indicate end of the list, a call to Prev() will return true and move the iterator to last item in the list.
	// (The opposite also applies when Prev() returns false to indicate there are no more preceding items).
	Next() (valid bool)
	Prev() (valid bool)
	// Returns the index of the current position of the iterator. Must be called on valid iterator.
	Pos() (index int)
	// Insert item before current position of the iterator. Returns the index of the newly inserted item.
	// The item to which the iterator points to does not change but it's position in the list will have incremented.
	// Must be called on valid iterator.
	Insert() (index int)
	// Removes list item at position of iterator.
	// Changes the state of the iterator to invalid until a subsequent call to Next()/Prev() returns true.
	// Must be called on valid iterator.
	Remove()
	//	Err() (err error)
}
...
type Interface interface {
	...
	// The given position will be the position of the iterator after a Next() or Prev() call.
